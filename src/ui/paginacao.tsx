...
export function Paginacao({ caminho, params, pagina, total, porPagina }: { caminho: string; params: Record<string, string | string[] | undefined>; pagina: number; total: number; porPagina: number }) {
  ...
  const classe = 'inline-flex items-center gap-1.5 rounded-lg border border-wr-borda px-3 py-1.5 text-[13px] no-underline hover:bg-wr-fundo';
  return (
    <nav aria-label="Paginação" className="flex flex-wrap items-center justify-between gap-2 border-t border-wr-borda px-4 py-3 text-[13px]">
      ...
      <div className="flex gap-2">
        ...
        {pagina > 1 ? <Link className={classe} href={href(pagina - 1)} rel="prev">‹ Anterior <IndicadorLink /></Link> : null}
        {pagina < paginas ? <Link className={classe} href={href(pagina + 1)} rel="next">Próxima › <IndicadorLink /></Link> : null}
      </div>
    </nav>
  );
}
