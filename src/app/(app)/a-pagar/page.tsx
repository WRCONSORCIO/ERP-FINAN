...
export default async function APagar({ searchParams }: { searchParams: Promise<Params> }) {
  ...
  return (
    ...
    >
      ...
      <Secao titulo="Folhas" descricao="Folha fechada não muda mais. Ao pagar, informe a data, o valor pago e o comprovante; se o valor for diferente, fica registrado." semPadding>
        {lista.length === 0 ? <EstadoVazio titulo="Nenhuma folha fechada ainda" /> : (
          <div className="tabela-quadro">
            <table className="tabela">
              <thead><tr><th>Mês</th><th>Fechada em</th><th className="direita">Comissões</th><th className="direita">Total</th><th>Situação</th><th>Exportar</th>{podeEditar ? <th>Pagamento</th> : null}</tr></thead>
              <tbody>
                {lista.map((f) => (
                  <tr key={f.id}>
